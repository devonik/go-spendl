...
import { AisClearRefinements, AisRangeInput, AisRefinementList } from 'vue-instantsearch/vue3/es'

const showFilter = ref(false)

function formatMinValue(minValue: string, minRange: string) {
  return minValue !== null && minValue !== minRange ? minValue : ''
}
function formatMaxValue(maxValue: string, maxRange: string) {
    ...
    <Transition mode="out-in">
      <div v-if="showFilter" class="flex flex-col gap-3">
        ...
        <div>
          ...
          <AisRefinementList attribute="group" :limit="5">
              ...
              <UCheckbox v-model:model-value="item.isRefined" class="my-1" :label="item.value" variant="card" @change="refine(item.value)">
                <template #label>
                  <div class="flex gap-3">
                    {{ $t(`search.productGroups.${item.value}`) }} <UBadge variant="outline">
                      {{ item.count }}
                    </UBadge>
                  </div>
                </template>
        ...
        <div>
          ...
          <AisRefinementList attribute="brand" :limit="5">
            <template #item="{ item, refine }">
              <UCheckbox v-model:model-value="item.isRefined" class="my-1" :label="item.value" variant="card" @change="refine(item.value)">
                <template #label>
                  <div class="flex gap-3">
                    {{ item.value }} <UBadge variant="outline">
                      {{ item.count }}
                    </UBadge>
                  </div>
                </template>
        ...
        <div>
          ...
          <AisRefinementList attribute="colors" :limit="5">
            <template #item="{ item, refine }">
              <UCheckbox v-model:model-value="item.isRefined" class="my-1" :label="item.value" variant="card" @change="refine(item.value)">
                <template #label>
                  <div class="flex gap-3">
        ...
        <div>
          ...
          <AisRangeInput attribute="price" class="flex gap-2">
