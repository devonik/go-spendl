...
- Hygiene crons (`vercel.json`): `/api/cron/check-satsback-stores` daily at 04:00 UTC diffs overrides against the live Satsback catalog and Slacks stale slugs; `/api/cron/purge-disabled-algolia-records` at 05:00 UTC deletes Algolia records for `crawl.crawlable: false` shops. Both auth via `Authorization: Bearer ${CRON_SECRET}`. Ad-hoc CLIs: `pnpm stores:disable-missing`, `pnpm algolia:purge-disabled`, `pnpm stores:dedupe` (dry-run by default, `--apply` to act).
