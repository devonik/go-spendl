...
- Hygiene crons (`vercel.json`): `/api/cron/check-satsback-stores` daily at 04:00 UTC diffs overrides against the live Satsback catalog **in both directions** and Slacks the result; `/api/cron/purge-disabled-algolia-records` at 05:00 UTC deletes Algolia records for `crawl.crawlable: false` shops. Both auth via `Authorization: Bearer ${CRON_SECRET}`. Ad-hoc CLIs: `pnpm stores:dedupe`, `pnpm stores:prune-delisted`, `pnpm algolia:purge-disabled`, `pnpm algolia:evict-stale` (all dry-run by default, `--apply` to act).
