...
- Results come back via `POST /api/crawl/webhook` (secured by `CRAWL_WEBHOOK_SECRET`). Webhook normalises: `imageAlt → name` fallback (Shopify), model code extracted from `»…«` in the name, price stripped of screen-reader labels, relative `productUrl` resolved against the crawled search URL's origin. Any 4xx/5xx counts as a failed crawl (a blocked shop looks identical to one with no hits) — except the single status a shop opts out via `crawl.emptyResultStatus`, for shops that serve an error page instead of an empty listing when a search has zero matches (biggreensmile.de 404s). Check what the error page's `baseSelector` extracts before setting it: a not-found page carrying recommendation tiles would otherwise be indexed as search results.
...
- **Schemas rot per-field.** Shops on CSS-module bundlers (Next.js etc.) emit hashed class names like `.Search_cur-money__nlYZ1`; a hash rotates when that module's contents change, so one field silently goes empty while the rest keep working. Prefer a semantic anchor (`[data-part="discount-price"]`, `:has()`) when the DOM offers one, or pair the hash with a structural fallback via a CSS group (`.Search_product-name__YXd1y, p:nth-of-type(2)`). Exception: `baseSelector` usually has to keep its hash — the hash-free equivalent tends to over-match banners, nav tiles and recommendation carousels. **The most durable form is a substring match on the stable middle:** a CSS-module class is `<prefix>__<name>_<hash>`, and only the hash rotates, so `[class*="__pricing"]` outlives the shop's next deploy while `._tp-algolia-product-card__pricing_1f7zk_231` does not (running-point). Structural selectors (`h3 a`) and unhashed framework classes (`li.ais-Hits-item` from InstantSearch, `img.tp-image__img`) are equally safe when available.
- **A schema fix counts as verified only after a real extraction run.** Post the schema to `$CRAWL4AI_URL` (set in CI, sandboxes and agent runs — `http://localhost:11235` is only the local Docker default, never assume it) against the shop's live search URL with the crawler config from `server/api/crawl/index.post.ts` mirrored (`scan_full_page`, `Sec-CH-UA*` headers, `cache_mode: BYPASS`, `delay_before_return_html`), and report before/after coverage per field as `n/total` items. Never infer a selector from unrendered HTML — many shops render their listings client-side (eufy's raw HTML contains zero product cards), so `curl` alone will tell you the page is empty. When `baseSelector` changes, check the **item count** too, not just field coverage: an over-permissive base quietly pulls in banners and nav tiles. Item counts drift between runs because of lazy loading, so compare on the shared set (join by `productUrl`) rather than on absolute totals.
...
- Hygiene crons (`vercel.json`): `/api/cron/check-satsback-stores` daily at 04:00 UTC diffs overrides against the live Satsback catalog **in both directions** and Slacks the result; `/api/cron/purge-disabled-algolia-records` at 05:00 UTC deletes Algolia records for `crawl.crawlable: false` shops. Both auth via `Authorization: Bearer ${CRON_SECRET}`. Ad-hoc CLIs: `pnpm stores:disable-missing`, `pnpm algolia:purge-disabled`, `pnpm stores:dedupe`, `pnpm stores:prune-delisted` (all dry-run by default, `--apply` to act).

**Satsback rotates slug suffixes.** The same shop comes back as `sixt-2` having been `sixt-6` (it moves in both directions, so it isn't a version counter). The live catalog itself holds no duplicates — 873 stores, zero repeated names, images or slug bases — but the spreadsheet keeps the old row and gains a new one, so pairs accumulate in `store-overrides.json` and the colleague marks one `DOPPELT`. Since `extendStores()` merges by slug, the stale twin's curation stops applying and the store silently falls back to `categories.other`. `pnpm stores:dedupe` reconciles this: for each family of slugs sharing a base at most one member is live, and that member is the answer. It writes `tmp/dedupe-plan.csv` for the colleague — the Germany sheet needs the same corrections, since `stores:sync` rebuilds from it and would otherwise undo `--apply`.
...
- **Overrides ship at build time, not runtime.** `store-overrides.json` is a static ESM import, so it's compiled into the server bundle — schema/category edits reach production only via a deploy, and there's nothing to invalidate. The 24h `cachedStores` TTL doesn't gate them: Nitro mounts `cache:` to the filesystem for `devStorage` only, so in production it's in-memory per function instance and a new deployment starts empty. That TTL only bounds staleness of the *Satsback upstream* list within one deployment.
